...
import type { employeeUpdateDTO } from '@/DTOs/admin/employee/EmployeeUpdateDTO'
import type { employeeI } from '@/interfaces/admin/employee/employeeInterface'
import { editEmployeeSchema } from '@/schemas/admin/employee/editEmployeeValidationSchema'
...
useBreadcrumb(() => [
  ...
  { name: employee.value ? `Editar - ${employee.value.user.name}` : 'Editar' },
])

const { meta, handleSubmit, errors, defineField, setErrors, setFieldValue, resetForm } = useForm({
...
const loadEmployee = async () => {
  try {
    ...
    resetForm({
      values: {
        name: employee.value?.user.name ?? '',
        last_name: employee.value.user.last_name ?? '',
        phone: employee.value?.phone.number ?? '',
        document_type: employee.value?.document.type ?? '',
        document_number: employee.value?.document.number ?? '',
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    await employeeService.update(payload, id)
    Swal.close()
...
<template>
  ...
  >
    <form action="" method="POST" @submit="onSubmit">
      ...
      <div class="mb-4" v-if="employee?.position != 'admin'">
        <label for="position" class="block mb-2 font-medium text-gray-900 dark:text-gray-200">
          Posición
        </label>
        <select
      ...
      <div class="mb-4">
        <label for="salary" class="block mb-2 font-medium text-gray-900 dark:text-gray-200">
          Sueldo
        </label>
        <input
          v-model="salary"
