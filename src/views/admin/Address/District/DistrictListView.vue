...
const districtsList = computed(() => districts.value?.data ?? [])
...
<template>
  <div class="flex justify-end">
    <ButtonCreate route="admin.address.districts.create" />
  </div>
  <AnimationLoader v-if="isLoading" />
  <div v-else-if="districtsList.length != 0">
      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        >
          <tr>
            ...
            <th scope="col" class="px-6 py-3">Costo Envío</th>
        ...
        <tbody>
          <tr
            :class="[
              ...
              index != districtsList.length - 1
                ? 'border-b dark:border-gray-700 border-gray-200'
                : '',
            ]"
            v-for="(district, index) in districtsList"
            :key="index"
          >
            <th
              scope="row"
            ...
            <td class="px-6 py-4">S/. {{ district.delivery_price }}</td>
            <td>
              <BadgeStatus :status="district.status" />
            </td>
            <td class="px-6 py-4 text-right">
              <div class="flex justify-around">
                <router-link
                  :to="{ name: 'admin.address.districts.show', params: { id: district.id } }"
                >
                  <font-awesome-icon icon="fa-solid fa-eye" size="xl" class="text-green-400" />
                </router-link>
                <router-link
                  :to="{ name: 'admin.address.districts.edit', params: { id: district.id } }"
                >
                  <font-awesome-icon
                ...
                <ToggleSwitch
                  ...
                  @update:status="() => updateStatus(district.id, district.status)"
                />
              </div>
            </td>
          </tr>
  ...
  <InfoAlert v-else message="Todavía no hay distritos registrados" />
</template>
